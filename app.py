...
def obteniendo_df_monte():
    """Función para conectar a S3 y obtener el DataFrame de Monte de Piedad."""
    session = create_aws_session('./credentials_aws.txt')
    s3_client = session.client('s3')

    current_date = datetime.now().strftime('%Y_%m')
    bucket_name = 's3-pernexium-report'
    prefix = f'raw/monte/replica_dynamo_chatbot/{current_date}/'
    response = s3_client.list_objects_v2(Bucket=bucket_name, Prefix=prefix)
    files = response.get('Contents', [])
    ...
    if files: 
        ...
        if isinstance(json_data, list):
            df_monte = pd.DataFrame(json_data)
        else:
            df_monte = pd.json_normalize(json_data)
        
        conversations = []
        for index, item in tqdm(df_monte.iterrows(), total=df_monte.shape[0]):
            temp_item = []
            for k in item['messages']:
                if 'content' not in k.keys():
                    continue
        ...
        df_monte_conversations = data

        return df_monte, df_monte_conversations
    else:
        print("No se encontraron archivos para la fecha actual.")
        return None, None
...
def inicio_page():
    ...
    passwords = {
        "developer": ["BanCoppel", "Monte de Piedad"],     # CAMBIAR IGUAL EN def solicitar_datos_page():
        "campana1": ["BanCoppel"],                         # CAMBIAR IGUAL EN def solicitar_datos_page():
        "campana2": ["Monte de Piedad"]                    # CAMBIAR IGUAL EN def solicitar_datos_page():
    }
    ...
    if not st.session_state['authenticated']:
        ...
        if password in passwords:
            ...
            st.session_state['user_role'] = password

            if password == 'developer':                    # CAMBIAR IGUAL EN def solicitar_datos_page():
                st.success("¡Contraseña correcta! ¡Bienvenido Developer!")
            elif password == 'campana1':                   # CAMBIAR IGUAL EN def solicitar_datos_page():
            ...
            elif password == 'campana2':                   # CAMBIAR IGUAL EN def solicitar_datos_page():
                st.success("¡Contraseña correcta! Puedes acceder a la página del Dashboard, pero primero solicita los datos en la página de 'Solicitar Datos'.")
        elif password:
            st.warning("Contraseña incorrecta. Inténtalo de nuevo.")
    else:
        if st.session_state['user_role'] == 'developer':   # CAMBIAR IGUAL EN def solicitar_datos_page():
            st.markdown("<p>¡Bienvenido Developer!</p>", unsafe_allow_html=True)
        elif st.session_state['user_role'] == 'campana1':  # CAMBIAR IGUAL EN def solicitar_datos_page():
            st.markdown("<p>¡Bienvenido al Dashboard de BanCoppel!</p>", unsafe_allow_html=True)
        elif st.session_state['user_role'] == 'campana2':  # CAMBIAR IGUAL EN def solicitar_datos_page():
            st.markdown("<p>¡Bienvenido al Dashboard de Monte de Piedad!</p>", unsafe_allow_html=True)
...
def solicitar_datos_page():
    ...
    if st.session_state['user_role'] == "developer":    # CAMBIAR IGUAL EN def inicio_page():
        opciones_disponibles = ['BanCoppel', 'Monte de Piedad']
    elif st.session_state['user_role'] == "campana1":   # CAMBIAR IGUAL EN def inicio_page():
        opciones_disponibles = ['BanCoppel']
    elif st.session_state['user_role'] == "campana2":   # CAMBIAR IGUAL EN def inicio_page():
        opciones_disponibles = ['Monte de Piedad']

    if opciones_disponibles:
        opcion = st.selectbox(
            'Selecciona la base de datos a cargar:',
            ['Ninguna'] + opciones_disponibles
        )
        ...
        elif opcion == 'Monte de Piedad':
            if 'df_monte' not in st.session_state or 'df_monte_conversations' not in st.session_state:
                with st.spinner("Cargando base de datos de Monte de Piedad desde S3, por favor espera..."):
                    df_monte, df_monte_conversations = obteniendo_df_monte()
                    
                    if df_monte is not None and df_monte_conversations is not None:
                        st.session_state.df_monte = df_monte
                        st.session_state.df_monte_conversations = df_monte_conversations
                        st.success("¡Datos de Monte de Piedad cargados con éxito, puedes ver el dashboard ahora!")
                    else:
                        st.session_state.df_monte = None  
                        st.session_state.df_monte_conversations = None
                        st.warning("No se pudo cargar el DataFrame de Monte de Piedad desde S3.")
            else:
                df_monte = st.session_state.df_monte
                df_monte_conversations = st.session_state.df_monte_conversations
    ...
    if 'df_monte' in st.session_state and st.session_state.df_monte is not None:
        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown("<h1>Base de Datos de <span style='color: #27A3D7;'>Monte de Piedad</span></h1>", unsafe_allow_html=True)
        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown("<h1>Base <span style='color: #27A3D7;'>Original</span></h1>", unsafe_allow_html=True)
        st.write(st.session_state.df_monte)
    
    if 'df_monte_conversations' in st.session_state and st.session_state.df_monte_conversations is not None:
        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown("<h1>Base de <span style='color: #27A3D7;'>Conversaciones</span></h1>", unsafe_allow_html=True)
        st.write(st.session_state.df_monte_conversations)
